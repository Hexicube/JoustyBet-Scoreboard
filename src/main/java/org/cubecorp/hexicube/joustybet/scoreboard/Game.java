...
public class Game implements ApplicationListener {
                ...
                try {
                    ...
                    sock.on(Socket.EVENT_CONNECT, new Emitter.Listener() {
                        ...
                        public void call(Object... objects) {
                            ...
                            renderCounter = 2;
                            // Don't set fullUpdate, so that it does a full
                            // update if data_update was recently called.
                        }
                    });
                    sock.connect();
                }
    ...
    public void render() {
        ...
        synchronized(betters) {
            ...
            for(PlayerCol c : cols)
                counts.add(new PlayerColCount(c));
            ...
            if(numToShow > 0) {
                ...
                for(int a = 0; a < 15; a++) {
                    if(a < numToShow) {
                        ...
                        if(acc >= topAcc)
                            tier = 4;
                        else if(acc >= topHalfAcc)
                            tier = 3;
                        else if(acc >= halfAcc)
                            tier = 2;
                        else if(acc >= bottomHalfAcc)
                            tier = 1;
                        else tier = 0;
                        
                        int place = a;
                        for(int z = a; z >= 0; z--) {
                            if(betters.get(z).uncertaintyAcc == acc)
                                place = z;
                            else break;
                        }
                ...
                for(int a = numToShow - 2; a > 0; a--) {
                    if(bottomChain < 2)
                        bottomChain = betters.get(a).streak;
                    else break;
                }
                ...
                for(int a = 0; a < 15; a++) {
                    if(a < numToShow) {
                        ...
                        if(chain >= topChain)
                            tier = 4;
                        else if(chain >= topHalfChain)
                            tier = 3;
                        else if(chain >= halfChain)
                            tier = 2;
                        else if(chain >= bottomHalfChain)
                            tier = 1;
                        else tier = 0;
                        
                        int place = a;
                        for(int z = a; z >= 0; z--) {
                            if(betters.get(z).streak == chain)
                                place = z;
                            else break;
                        }
